...
def run_query(tokenizer, model, messages, temperature=0.1, max_new_tokens=512, **kwargs,):
  ...
  with torch.no_grad():
    ...
    response = output.split("<|eot_id|>")[-2].strip()  # Get the part before the last <|eot_id|>

    # Remove unwanted tokens like <|start_header_id|> and <|end_header_id|>
    response = response.replace("<|start_header_id|>", "").replace("<|end_header_id|>", "").strip()

    # Further clean up in case there are any other markers or whitespace
...
def initialise_and_run_model(save_name, input_stage_1, model_name):
  ...
  model = AutoModelForCausalLM.from_pretrained(model_name,token='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',
                                               device_map="auto")

  tokenizer = AutoTokenizer.from_pretrained(model_name,token='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',
                                            device_map="auto")
...
if __name__ == "__main__":
  ...
  output_file = "output/RAGB.json"
  
  input_stage_1 = 'output/rerankerC.json'
